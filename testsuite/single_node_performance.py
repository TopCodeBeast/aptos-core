...
EXPECTED_TPS = {
    ...
    ("account-generation", False): 10000.0,
    ("account-generation", True): 15500.0,
    ("create-new-account-resource", False): 12300.0,
    ("modify-global-resource", False): 4000.0,
    ("modify-ten-global-resources", False): 10700.0, 
    ("large-module-working-set-no-op", False): 2700.0,
    ("publish-package", False): 140.0,
    ("batch100-transfer", False): 300,
    ("batch100-transfer", True): 500,
    # ("token-v1ft-mint-and-store", False): 1000.0,
    ("token-v1ft-mint-and-transfer", False): 1800.0,
    ("token-v1nft-mint-and-transfer-sequential", False): 1150.0,
...
NUM_ACCOUNTS = max([2000000, 20 * NUM_BLOCKS * BLOCK_SIZE])
...
with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    for (transaction_type, use_fake_executor), expected_tps in EXPECTED_TPS.items():
        ...
        use_fake_executor_row_str = "native" if use_fake_executor else "VM"
        row = [transaction_type, use_fake_executor_row_str, cur_block_size, expected_tps]
        gas_row = [transaction_type, use_fake_executor_row_str, cur_block_size]

        use_fake_executor_str = "--use-fake-executor" if use_fake_executor else ""
        common_command_suffix = f"{use_fake_executor_str} --generate-then-execute --transactions-per-sender 1 --block-size {cur_block_size} --use-state-kv-db --use-sharded-state-merkle-db run-executor --transaction-type {transaction_type} --main-signer-accounts {MAIN_SIGNER_ACCOUNTS} --additional-dst-pool-accounts {ADDITIONAL_DST_POOL_ACCOUNTS} --data-dir {tmpdirname}/db  --checkpoint-dir {tmpdirname}/cp"
        for concurrency_level in EXECUTION_ONLY_CONCURRENCY_LEVELS: 
            ...
            achieved_tps[transaction_type][concurrency_level] = int(round(tps))
            achieved_gps[transaction_type][concurrency_level] = int(round(gps))
            row.append(tps)
            gas_row.append(gps)
        ...
        achieved_tps[transaction_type][0] = int(round(tps))
        achieved_gps[transaction_type][0] = int(round(gps))
        row.append(tps)
        gas_row.append(gps)
        ...
        print(tabulate(
            rows, 
            headers=["transaction_type", "executor", "block_size", "expected t/s"] 
            + [f"exe_only {concurrency_level}" for concurrency_level in EXECUTION_ONLY_CONCURRENCY_LEVELS]
            + ["t/s"]))
        ...
        if tps < expected_tps * (1 - NOISE_FRACTION):
            errors.append(f"regression detected {tps} < {expected_tps} * {1 - NOISE_FRACTION}, {transaction_type} with {use_fake_executor_row_str} executor didn't meet TPS requirements")
        elif tps > expected_tps * (1 + NOISE_FRACTION):
            errors.append(f"perf improvement detected {tps} > {expected_tps} * {1 + NOISE_FRACTION}, {transaction_type} with {use_fake_executor_row_str} executor exceeded TPS requirements, increase TPS requirements to match new baseline")
