    ...
    V: TransactionBlockExecutor<BenchmarkTransaction> + 'static,
{
    ...
    let (pipeline, block_sender) = Pipeline::new(executor, version, pipeline_config.clone(), num_blocks);
    ...
    V: TransactionBlockExecutor<BenchmarkTransaction> + 'static,
{
    ...
    let (pipeline, block_sender) = Pipeline::new(executor, version, pipeline_config, 1 + num_new_accounts / block_size * 101 / 100);
